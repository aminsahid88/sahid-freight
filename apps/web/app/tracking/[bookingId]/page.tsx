    ...
    <div style={{ height: "100vh", background: "#0A1F44", fontFamily: "\'Helvetica Neue\', Arial, sans-serif", display: "flex", flexDirection: "column" as const, overflow: "hidden" }}>
      <style>{`
        ...
        @media (max-width: 640px) {
          .ptrack-header { padding: 10px 12px !important; gap: 8px; }
          .ptrack-header-left { gap: 8px !important; min-width: 0; flex: 1; }
          .ptrack-back-label { display: none; }
          .ptrack-back-btn { padding: 8px !important; min-width: 40px; min-height: 40px; justify-content: center; }
          .ptrack-title-block { min-width: 0; flex: 1; }
          .ptrack-title { font-size: 13px !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
          .ptrack-subtitle { font-size: 10px !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
          .ptrack-status-label { font-size: 10px !important; }
          .ptrack-bottom { padding: 12px 14px !important; max-height: 50vh; overflow-y: auto; }
          .ptrack-latlng { gap: 8px !important; }
          .ptrack-action-btn { min-height: 44px !important; padding: 12px !important; }
        }
      `}</style>

      {/* Header */}
      <div className="ptrack-header" style={{ padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.08)", flexShrink: 0, background: "#0A1F44", zIndex: 10 }}>
        <div className="ptrack-header-left" style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <button onClick={() => router.back()} aria-label="Go back" className="ptrack-back-btn" style={{ background: "rgba(255,255,255,0.08)", border: "none", borderRadius: "8px", padding: "7px 10px", color: "#F8FAFC", fontSize: "13px", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}>
            <ArrowLeft size={14} /> <span className="ptrack-back-label">Back</span>
          </button>
          <div className="ptrack-title-block">
            <div className="ptrack-title" style={{ fontSize: "14px", fontWeight: "700", color: "#F8FAFC" }}>{booking?.load?.title || "Live tracking"}</div>
            <div className="ptrack-subtitle" style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", marginTop: "1px" }}>{booking?.load?.pickupCity} → {booking?.load?.deliveryCity}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <span className="ptrack-status-label" style={{ fontSize: "11px", color: isSharing ? "#5BE3C4" : "rgba(255,255,255,0.4)", fontWeight: "700", letterSpacing: "0.5px" }}>
      ...
      {/* Bottom panel */}
      <div className="ptrack-bottom" style={{ background: "#0A1F44", padding: "16px 20px", flexShrink: 0, zIndex: 10 }}>
        ...
        {location && (
          <div className="ptrack-latlng" style={{ display: "flex", gap: "10px", marginBottom: "12px" }}>
        ...
        {canTakeActions && (
          <div style={{ display: "flex", flexDirection: "column" as const, gap: "8px" }}>
            {!isInTransit && !isDelivered && (
              <button onClick={handleStartJourney} disabled={actionLoading} className="ptrack-action-btn"
            ...
            {isInTransit && !isDelivered && (
              <>
                <button onClick={isSharing ? stopSharing : startSharing} className="ptrack-action-btn"
                ...
                <button onClick={handleMarkDelivered} disabled={actionLoading} className="ptrack-action-btn"
        ...
        {!canTakeActions && (
          <div>
            ...
            {isInTransit && location && (
              <a href={"https://maps.google.com/?q=" + location.lat + "," + location.lng} target="_blank" rel="noopener noreferrer" className="ptrack-action-btn"
