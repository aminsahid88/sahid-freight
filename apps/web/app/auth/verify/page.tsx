...
export default function VerifyPage() {
  ...
  return (
    ...
    <div style={{ minHeight: "100vh", display: "flex", fontFamily: "'Helvetica Neue', Arial, sans-serif", background: "#f0ebe0" }}>
      ...
      {/* Left panel */}
      <div className="verify-left" style={{ width: "52%", background: "#1a2744", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", padding: "52px 60px" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: "42px", fontWeight: "800", color: "#f0ebe0", lineHeight: "1.15", margin: "0 0 24px", letterSpacing: "-1.5px" }}>
            One step<br />
            <span style={{ color: "#c8901e" }}>away.</span>
          </h1>
          ...
          <div style={{ marginTop: "48px", background: "rgba(240,235,224,0.04)", border: "1px solid rgba(240,235,224,0.06)", borderRadius: "12px", padding: "20px" }}>
            <div style={{ fontSize: "11px", color: "#c8901e", letterSpacing: "2px", textTransform: "uppercase" as const, marginBottom: "8px" }}>Sent to</div>
            <div style={{ fontSize: "18px", fontWeight: "700", color: "#f0ebe0", fontFamily: "monospace" }}>{user?.email}</div>
          </div>
        </div>
      ...
      <div className="verify-right" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "48px" }}>
        <div style={{ width: "100%", maxWidth: "400px" }}>
          <div className="verify-card" style={{ background: "#fff", borderRadius: "20px", padding: "48px 44px", boxShadow: "0 2px 4px rgba(26,39,68,0.04), 0 16px 48px rgba(26,39,68,0.10)", border: "1px solid rgba(26,39,68,0.06)" }}>

            {/* Icon */}
            <div style={{ width: "56px", height: "56px", background: "#f0ebe0", borderRadius: "16px", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "24px", color: "#1a2744" }}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg></div>

            <h2 style={{ fontSize: "24px", fontWeight: "800", color: "#1a2744", margin: "0 0 8px", letterSpacing: "-0.5px" }}>
              Verify your number
            </h2>
            <p style={{ color: "#9e9890", fontSize: "14px", margin: "0 0 36px", lineHeight: "1.6" }}>
              Enter the 6-digit code sent to <strong style={{ color: "#1a2744" }}>{user?.email}</strong>
            </p>
              ...
              {otp.map((digit, i) => (
                <input
                  ...
                  style={{
                    ...
                    fontWeight: "800",
                    color: "#1a2744",
                    background: digit ? "#f0ebe0" : "#faf8f4",
                    border: `2px solid ${digit ? "#1a2744" : "#e8e3d8"}`,
                    borderRadius: "12px",
                    outline: "none",
            ...
            <button
              ...
              style={{ width: "100%", background: loading || otp.some((d) => d === "") ? "#e8e3d8" : "#1a2744", border: "none", borderRadius: "10px", padding: "15px", color: loading || otp.some((d) => d === "") ? "#aaa" : "#f0ebe0", fontSize: "15px", fontWeight: "700", cursor: loading || otp.some((d) => d === "") ? "not-allowed" : "pointer", transition: "all 0.2s", marginBottom: "20px" }}
            >
            ...
            <div style={{ textAlign: "center" as const }}>
              {canResend ? (
                <button onClick={handleResend} disabled={resending} style={{ background: "none", border: "none", color: "#c8901e", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}>
                  {resending ? "Sending..." : "Resend code"}
                </button>
              ) : (
                <p style={{ color: "#9e9890", fontSize: "14px", margin: 0 }}>
                  Resend code in <span style={{ fontWeight: "700", color: "#1a2744" }}>{countdown}s</span>
                </p>
              )}
            </div>

            <div style={{ marginTop: "24px", paddingTop: "20px", borderTop: "1px solid #f0ede6", textAlign: "center" as const }}>
              <a href="/auth/login" style={{ color: "#9e9890", fontSize: "13px", textDecoration: "none" }}>
                Wrong number? <span style={{ color: "#c8901e", fontWeight: "600" }}>Sign in again</span>
              </a>
            </div>
          </div>

          {/* Dev hint */}
          <div style={{ marginTop: "16px", background: "rgba(26,39,68,0.06)", borderRadius: "10px", padding: "12px 16px", textAlign: "center" as const }}>
            <p style={{ color: "#9e9890", fontSize: "12px", margin: 0 }}>
              💡 Check your API terminal for the OTP code
            </p>
          </div>
        </div>
      </div>
