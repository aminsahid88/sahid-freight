/**
 * Human-readable error mapper. Every catch(...) in the web app should
 * pass its error through formatApiError so users never see raw axios
 * text, stack traces, "Failed to fetch", null / undefined, or server
 * error codes.
 *
 * Mirrors apps/mobile/src/lib/errors.ts.
 */
...
export function formatApiError(
  ...
  //   Axios sets ERR_NETWORK when the request never reached the server;
  //   fetch throws "Failed to fetch" / "NetworkError". Timeouts get their
  //   own message.
  if (
    e?.code === "ERR_NETWORK" ||
    e?.code === "ENOTFOUND" ||
  ...
  if (status === 400) {
    // 400s usually carry a validation reason from the API; prefer it
    // when it looks like a real sentence (not a raw code).
  ...
  if (status === 409) {
    // Conflict — typical "already exists" / "already booked" cases.
    if (isDisplayableServerMessage(rawMsg)) return capitalize(rawMsg!);
...
/** Returns true when a raw server message is safe to show a user. */
function isDisplayableServerMessage(msg: string | undefined): msg is string {
  ...
  if (/^\[.*\]$/.test(t)) return false;                      // array-shaped
  if (/^\{.*\}$/.test(t)) return false;                      // JSON-shaped
  if (/failed to fetch|network error|econnaborted/i.test(t)) return false;
  if (t.length > 200) return false;                          // suspicious length
  return true;
}
